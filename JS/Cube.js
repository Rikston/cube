...
String.prototype.__FindEntry = function(word, EntryNumber) {
  ...
  for (var i = 0; true; ) {
    ...
    if (result != -1) {
      ++i;
      if (i == EntryNumber) return result;
    } else return result;
  }
};
...
function Interface(_obj) {
  ...
  Object.defineProperties(this, {
    ...
    margin: {
      get: function() {
        return this.obj.style.margin;
      },
      set: function(Margin) {
        this.obj.style.margin = Margin;
      }
    },
    background: {
      get: function() {
...
function Side(settings) {
  ...
  this.style.backgroundColor =
    settings.backgroundColor || "rgba(159, 18, 16, .5)";
...
function Cube(_anchor, settings) {
  if (_anchor) {
    ...
    this.style.transformOrigin =
      "50% 50% " + parseInt(this.style.width) / 2 + "px";
    ...
    this.obj.onmousewheel = function(e) {
      ...
      this.obj.style.width =
        parseInt(this.obj.style.width) + -e.deltaY / 5 + "px";
      this.obj.style.height =
        parseInt(this.obj.style.height) + -e.deltaY / 5 + "px";
    ...
    else this._anchor.style.width = this._anchor.dataset.anchorWidth;

    if (!this._anchor.hasAttribute("data-anchor-height"))
      this._anchor.style.height = settings.anchorHeight || "500px";
    else this._anchor.style.height = this._anchor.dataset.anchorHeight;
    ...
    this._anchor.onmousemove = function(e) {
      if (this._switchRotation) {
        this.obj.style.transform =
          "rotateX(" +
          -(this._rotateY += e.pageY - this._differenceY) +
          "deg) rotateY(" +
          (this._rotateX += e.pageX - this._differenceX) +
          "deg)";
        this._differenceX = e.pageX;
        this._differenceY = e.pageY;
...
Cube.prototype.TransformOriginCorrection = function() {
  this.obj.style.transformOrigin =
    "50% 50% " + parseInt(this.sides[0]._style.width) / 2 + "px";
  for (var i = 0; i < 6; ++i) {
    this.sides[i].style.transformOrigin =
      "50% 50% " + parseInt(this.sides[i]._style.width) / 2 + "px";
  }
};
...
function SettingsPanel(settings) {
  ...
  this.type = (
    settings.type ||
    (this._anchor ? this._anchor.dataset.panelPosition : 0) ||
    "top"
  ).toLowerCase();
  ...
  this.background =
    settings.background || settings.backgroundColor || "#eff1f1";
  this.style.boxSizing = settings.boxSizing || "border-box";
  this.display = settings.display || "flex";
  this.margin =
    (settings.marginTop || "0px") +
    " " +
    (settings.marginRight || "0px") +
    " " +
    (settings.marginBottom || "0px") +
    " " +
    (settings.marginLeft || "0px");
  ...
  if (this.type == "left" || this.type == "right") {
    ...
    this.style.width =
      settings.width ||
      (this._anchor ? this._anchor.dataset.panelWidth : 0) ||
      "100px";
    this.style.top = "0px";
    if (this.type == "left") {
    ...
    if (this.type == "top" || this.type == "bottom") {
      ...
      this.style.height =
        settings.height ||
        (this._anchor ? this._anchor.dataset.panelHeight : 0) ||
        "80px";
  ...
  if (settings.alignSelf) this.style.alignSelf = settings.alignSelf;
  ...
  if (this._anchor) {
    ...
    this._anchor.style.height =
      styleAn.height == "0px" ? "100%" : styleAn.height;
    this._anchor.appendChild(this.obj);
  } else document.body.appendChild(this.obj);
}
SettingsPanel.prototype.Add = function(tool) {
  if (tool.associationCube) this.obj.appendChild(tool.obj);
  else this.obj.appendChild(tool);
...
function Slider(settings) {
  ...
  if (this.type == "horizontal") {
    ...
    var endMove =
      this.width -
      parseInt(this._circle.style.marginLeft) * 2 -
      this._circle.width;
  } else {
    if (this.type == "vertical") {
      ...
      var endMove =
        this._line.height -
        parseInt(this._circle.style.marginTop) * 2 -
        this._circle.height;
    } else {
...
function Switcher(settings) {
  ...
  this._circle =
    settings.circle ||
    new Circle({
      radius: parseInt(this.style.height) - 4 + "px",
      marginLeft: "2px",
      marginTop: "2px"
    });
  ...
  var endMove =
    ...
    this._circle.width -
    parseInt(this._circle.style.marginLeft) * 2;
  if (Sign) this._circle.obj.style.left = endMove + "px";
  var Animation;
  this.onchange = function(a) {};
  this.A = function() {
    return Animation;
...
function SliderForColorPicker(settings) {
      ...
      if (beginMove < this._circle.left && this._circle.left < endMove)
        ...
        if (beginMove == this._circle.left && e.pageX > difference)
          ...
          if (endMove == this._circle.left && e.pageX < difference) {
            ...
            if (beginMove > this._circle.left)
              this._circle.left = beginMove + "px";
            if (endMove < this._circle.left) this._circle.left = endMove + "px";
          }
        }
      }
      let ImgRGBA = holst.getImageData(
        this._circle.left + this._circle.width / 2,
        6,
        10,
        10
      );
      this.spectrum.getContext("2d").fillRect(0, 0, 486, 10);
      this._circle.style.backgroundColor =
        "rgb(" +
        ImgRGBA.data[0] +
        ", " +
        ImgRGBA.data[1] +
  ...
  Object.defineProperty(this, "value", {
    get: function() {
      return this._circle.style.backgroundColor;
    }
  });
...
function HolstforColorSelection(settings) {
  ...
  var MMove = function(e) {
    if (switcherMove) {
      ...
      if (beginMove < this._circle.left && this._circle.left < endMoveX)
        ...
        if (this._circle.left == beginMove && e.pageX > differenceX) {
          ...
          if (this._circle.left == endMoveX && e.pageX < differenceX)
            this._circle.left =
              this._circle.left + e.pageX - differenceX + "px";
          else {
            if (this._circle.left < beginMove)
              this._circle.left = beginMove + "px";
            else {
              if (this._circle.left > endMoveX)
                this._circle.left = endMoveX + "px";
            }
          }
        }
      }
      if (beginMove < this._circle.top && this._circle.top < endMoveY) {
        ...
        if (this._circle.top == beginMove && e.pageY > differenceY) {
          ...
          if (this._circle.top == endMoveY && e.pageY < differenceY) {
            ...
            if (this._circle.top < beginMove)
              this._circle.top = beginMove + "px";
            else {
      ...
      differenceX = e.pageX;
      differenceY = e.pageY;
      let dat = s.getImageData(
        this._circle.left + this._circle.width / 2,
        this._circle.top + this._circle.height / 2,
        1,
        1
      );

      this._circle.style.backgroundColor =
        "rgb(" + dat.data[0] + ", " + dat.data[1] + ", " + dat.data[2] + ")";
      this.onchange(this.value);
    }
  }.bind(this);
  var MUp = function() {
  ...
  this.updata = function(color) {
    ...
    s.fillRect(0, 0, Holst.width, Holst.height);
    let RGBa = s.getImageData(
      this._circle.left + this._circle.width / 2 - 1,
      this._circle.top + this._circle.height / 2,
      1,
      1
    );
...
function ColorPicker(settings) {
  ...
  this._slider = new SliderForColorPicker({
    width: this.width - 50 + "px",
    anchor: this.obj,
    marginTop: "20px"
  });
  this._holst = new HolstforColorSelection({
    width: this.width + "px",
    anchor: this.obj,
    boxShadow: "0px 4px 5px 1px rgba(0, 0, 0, 0.62)"
  });
  ...
  Object.defineProperties(this, {
    color: {
      get: function() {
        return this._holst.value;
      }
    },
    onchange: {
      get: function() {
        return this._holst.onchange;
      },
      set: function(fun) {
  ...
  if (anchor) {
    ...
    if (!settings.element) {
      ...
      this.margin =
        ...
        (settings.marginLeft || "0px");
      this.background =
        settings.background || settings.backgroundColor || "rgb(183, 244, 244)";
    ...
    if (anchor.associationCube) {
      ...
      if (settings.parent) anchor.obj.appendChild(this.obj);
    } else {
      anchor.parentNode.appendChild(this.obj);
    }
    this.obj.onclick = function() {
      if (Actions.An == 0) Actions.Hide();
    }.bind(this);
